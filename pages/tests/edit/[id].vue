  ...
  <div class="test-edit-page">
    ...
    <div class="main-layout">
      ...
      <main class="content-area">
        ...
        <div v-else class="test-edit-content">
          <!-- ヘッダー -->
...
const loadTestData = async () => {
  ...
  try {
    ...
    // スキル情報を処理（後方互換性のため、skills配列またはskillIdをサポート）
    ...
    if (data.skills && Array.isArray(data.skills) && data.skills.length > 0) {
      ...
      data.skills.forEach((skill: any) => {
        selectedSkills.push({
          ...
          proficiencyLevel: skill.proficiencyLevelOnPass || 'circle'
        })
      })
      ...
      selectedSkills.push({
        ...
        proficiencyLevel: data.proficiencyLevelOnPass || 'circle'
      })
    }
    
    // スキルが選択されていない場合、空の選択行を1つ追加
    ...
    console.log('[TestEdit] Loaded test data:', testData.value)
  } catch (error) {
...
const handleSave = async () => {
  ...
  try {
    ...
    console.log('[TestEdit] Test updated successfully:', response)
    alert('テストを更新しました')
    ...
    alert('テストの更新に失敗しました: ' + (error.data?.message || error.message || '不明なエラー'))
  } finally {
